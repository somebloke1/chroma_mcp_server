...
"""
        ...
        if post_commit_path.exists():
            ...
            # Check if our test transition code is already there
            if "Checking for test transitions" not in existing_content:
                # Append our code to existing hook
                with open(post_commit_path, "a") as f:
                    f.write(test_transition_check)
                logger.info("Added test transition check to existing post-commit hook")
            else:
                logger.info("Test transition check already in post-commit hook")
        else:
            # Create new post-commit hook with standard content
            standard_post_commit = """#!/bin/bash
# Auto-generated by TestWorkflowManager
...
"""
            # Add the test transition check
            with open(post_commit_path, "w") as f:
                f.write(standard_post_commit + test_transition_check)
