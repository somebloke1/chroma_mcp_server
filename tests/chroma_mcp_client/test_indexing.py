...
def mock_chroma_client_tuple(mocker):
    ...
    mock_get_client_and_ef = mocker.patch(
        "chroma_mcp_client.indexing.get_client_and_ef",
        return_value=(mock_client, mock_embedding_func)
    )
...
def test_index_git_files_success(mock_index_file, mock_subprocess_run, temp_repo: Path, mocker):
    ...
    mock_index_file.assert_any_call(temp_repo / "src/main.py", temp_repo, "codebase_v1", mocker.ANY) # mocker.ANY for default suffixes
...
def test_index_paths_files_and_dirs(mock_index_file, mock_os_walk, temp_repo: Path, mocker):
    ...
    paths_to_index = {
        "src/main.py", # Relative path to a file
        "dir1"         # Relative path to the directory
    }
